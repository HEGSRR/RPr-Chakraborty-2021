...
acs_covid |> filter(is.na(bpov_pct)) |> 
  st_drop_geometry() |> 
  t() |> 
  kable() |> kable_styling()
```
...
saveRDS(covid_table, here("data", "derived", "public", "covid_table.RDS"))
```
...
covid_table <- readRDS(here("data", "derived", "public", "covid_table.RDS"))
state <- read_sf(here("data", "derived", "public", "acs.gpkg"), layer = "state")
```
...
Load data for GEE models

```{r load-derived, eval = FALSE}
acs_covid <- st_read(here("data", "derived", "public", "acs_covid_clusters.gpkg"))
```
...
# ggsave(here("results", "figures", "dotwhisker_reproduction.svg"), plot = geewhisker_fig)
...
geereanalysisresults <- bind_rows(
  mutate(gee_or_clus_rpdep$coefficients,
...
# generate create dot whisker plot
geewhisker_reanalysis_fig <- geereanalysisresults %>%
  filter(!grepl("intercept", term)) %>%
  dwplot(dodge_size = 0.6,
         vline = geom_vline(
           xintercept = 0,
...
# ggsave(here("results", "figures", "dotwhisker_reanalysis.svg"), plot = geewhisker_reanalysis_fig)
...
```{r save-gee-results, eval=FALSE}
allgeeresults <- allgeeresults |> bind_rows(geereanalysisresults) |> select(1:6)
allgeeresults |> write_csv(here("data", "derived", "public", "gee_results.csv"))
```
