...
Version 2.0 \| First Created July 7, 2021 \| Updated March 28, 2025
...
tm_spatialepi_cluster_risk_class <- tm_shape(acs_covid) +
  tm_polygons(fill = "cluster_class",
              fill.scale = tm_scale_categorical(
                              values = "brewer.oranges",
                              labels = class_freq$label),
...
satscan_rpr_gini <- satscan_rpr_gis_t %>% filter(GINI_CLUST == TRUE) %>% select(fips, gini_rr = CLU_RR)
satscan_rpr_hier <- satscan_rpr_gis_t %>% filter(GINI_CLUST == FALSE) %>% select(fips, hier_rr = CLU_RR)
...
tm_spatialepi_hier <-
  tm_shape(state) +
  tm_fill("gray98") +
  tm_shape(hier_clusters |> filter(!is.na(xcluster))) +
  tm_polygons(
    fill = "xcluster",
    fill.scale = tm_scale(
      values = c("wheat", "tomato", "thistle3")),
    fill.legend = tm_legend(title = "SaTScan Hierarchical and SpatialEpi Clusters"),
    col = "white",
    lwd = 0.2,
    col_alpha = 0.3
  ) +
...
tm_spatialepi_gini <-
  tm_shape(state) +
  tm_fill("gray98") +
  tm_shape(gini_clusters |> filter(!is.na(xcluster))) +
  tm_polygons(
    fill = "xcluster",
    fill.scale = tm_scale(
      values = c("wheat", "tomato", "thistle3")),
    fill.legend = tm_legend(title = "SaTScan GINI Optimized and SpatialEpi Clusters"),
    col = "white",
    lwd = 0.2,
    col_alpha = 0.3
  ) +
